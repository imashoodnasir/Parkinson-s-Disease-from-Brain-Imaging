...
def build_model(cfg, depth: int):
    m = cfg["model"]
    base = m["blocks_per_stage"]
    if depth < len(base):
        blocks = base[:depth]
    elif depth > len(base):
        blocks = base + [base[-1]] * (depth - len(base))
    else:
        blocks = base
...
def train_depth(cfg, cohort, depth):
...
def main():
    ...
    ap.add_argument("--depths", nargs="+", type=int, default=[2,3,4,5])
    args = ap.parse_args()
    ...
    out = {}
    out_dir = Path("results")/f"ablation_depth_{args.cohort}"
    out_dir.mkdir(parents=True, exist_ok=True)

    for d in args.depths:
        m = train_depth(cfg, args.cohort, d)
        out[f"L={d}"] = m
        print("L=", d, m)
